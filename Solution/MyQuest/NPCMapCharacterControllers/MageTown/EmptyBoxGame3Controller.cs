...
namespace MyQuest
{
    public class EmptyBoxGame3Controller : NPCMapCharacterController
    {
        Dialog EmptyOneDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z171);  //Declare the dialog variable
        ...
        public override void Interact()  //The interact method will activate when the player presses the A button(or the space bar) on the npc.
        {
            ...
            ScreenManager.Singleton.AddScreen(new DialogScreen(EmptyOneDialog, DialogScreen.Location.TopLeft));  //Brings the Dialog up, Location at the topleft.
        }
    }
}
