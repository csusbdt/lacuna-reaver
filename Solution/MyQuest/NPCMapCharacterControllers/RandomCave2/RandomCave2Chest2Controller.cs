...
namespace MyQuest
{
    public class RandomCave2Chest2Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.ZA286);

        public void Complete()
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(JadeStatue), 1);

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "RandomCave2Chest2",  //The name of the npc being modified
