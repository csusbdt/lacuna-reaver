...
namespace MyQuest
{
    public class CaveLabyrinthChest5Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z122);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(LargeEnergyPotion), 1);  //Adds a Large energy potion to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "CaveLabyrinthChest5",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest5",
                new Point(59,36),  //OpenChest3 is added at position (59,36). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
