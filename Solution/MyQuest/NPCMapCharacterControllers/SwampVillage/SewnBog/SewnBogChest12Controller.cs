...
namespace MyQuest
{
    public class SewnBogChest12Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z118);  //Declare the dialog variable

        public void Complete()
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(LargeHealthPotion), 1);  //Adds a Large Health Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "SewnBogChest12",  //The name of the npc being modified
