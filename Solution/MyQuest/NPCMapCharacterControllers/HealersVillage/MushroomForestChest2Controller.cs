...
namespace MyQuest
{
    public class MushroomForestChest2Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z086);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(SmallEnergyPotion), 1);  //Adds the Small Energy Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "MushroomForestChest2",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest2",
                new Point(47, 28),  //OpenChest1 is added at position (47,28). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
