...
namespace MyQuest
{
    public class MushroomForestChest5Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z087);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(SmallHealthPotion), 1);  //Adds the Large Health Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "MushroomForestChest5",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest5",
                new Point(43,12),  //OpenChest3 is added at position (43,12). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
