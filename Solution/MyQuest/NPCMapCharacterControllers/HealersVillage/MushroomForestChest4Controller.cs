...
namespace MyQuest
{
    public class MushroomForestChest4Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z088);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(SmallEnergyPotion), 1);  //Adds 2 Medium Energy Potions to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "MushroomForestChest4",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest4",
                new Point(7,47),  //OpenChest2 is added at position (7,47). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
