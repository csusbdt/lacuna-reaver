...
namespace MyQuest
{
    public class RandomCave1Chest2Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.ZA266);  //Declare the dialog variable

        public void Complete()
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(RubyRing), 1);

            Party.Singleton.GameState.Gold += 125;

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "RandomCave1Chest2",  //The name of the npc being modified
