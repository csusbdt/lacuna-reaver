...
namespace MyQuest
{
    public class ForbiddenCavernRoomCChestController : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.Z120);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(TrainingBelt), 1);  //Adds the Small Health Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "ForbiddenCavernRoomCChest",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest1",
                new Point(13,5),  //OpenChest1 is added at position (13,5). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
