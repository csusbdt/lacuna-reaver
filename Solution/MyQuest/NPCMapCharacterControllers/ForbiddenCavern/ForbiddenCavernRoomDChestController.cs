...
namespace MyQuest
{
    public class ForbiddenCavernRoomDChestController : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.ZA570);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(BloodMetalOre), 1);  //Adds the Small Energy Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Party.Singleton.CurrentMap.Name,  //Select the map where the modification is applied. Current map in this case.
                "ForbiddenCavernRoomDChest",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                ...
                "OpenChest1",
                new Point(0,22),  //OpenChest1 is added at position (0,22). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
