...
namespace MyQuest
{
    public class ForbiddenCavernRoomBGroundChest3Controller : NPCMapCharacterController
    {
        Dialog giveDialog = new Dialog(DialogPrompt.NeedsClose, Strings.ZA553);  //Declare the dialog variable
        ...
        {
            Party.Singleton.GameState.Inventory.AddItem(typeof(GreenMark), 1);  //Adds the Small Health Potion item to the inventory

            Party.Singleton.ModifyNPC(
                Maps.forbiddenCavernRoombGround,  //Select the map where the modification is applied. Current map in this case.
                "ForbiddenCavernRoomBGroundChest3",  //The name of the npc being modified
            ...
            Party.Singleton.ModifyNPC(
                Maps.forbiddenCavernRoombLedge,
                "ForbiddenCavernRoomBGroundChest3",
                Point.Zero,
                ModAction.Remove,
                true);

            Party.Singleton.ModifyNPC(
                Maps.forbiddenCavernRoombGround,
                "OpenChest3",
                new Point(18,9),  //OpenChest3 is added at position (18,9). Press D on the keyboard in the game to find the map name and the coordinates.
                ModAction.Add,
                true);
