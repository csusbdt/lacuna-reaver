...
namespace MyQuest
{
    public class Battle1SceneB : Scene
    {
        Dialog dialog;
        Dialog bandit1Dialog1 = 
            new Dialog(DialogPrompt.NeedsClose, Strings.ZA574);

        Dialog bandit2Dialog1 =
            new Dialog(DialogPrompt.NeedsClose, Strings.ZA575);

        Dialog bandit1Dialog2 =
            new Dialog(DialogPrompt.NeedsClose, Strings.ZA576);

        Dialog bandit2Dialog2 =
            new Dialog(DialogPrompt.NeedsClose, Strings.ZA577);

        Dialog bandit1Dialog3 =
            new Dialog(DialogPrompt.NeedsClose, Strings.ZA578);

        NPCMapCharacter bandit1;
        NPCMapCharacter bandit2;

        public Battle1SceneB(Screen screen)
            : base(screen)
        {
        }
        ...
        {
            CombatZone zone = CombatZonePool.dungeon3CaveBanditZone;
