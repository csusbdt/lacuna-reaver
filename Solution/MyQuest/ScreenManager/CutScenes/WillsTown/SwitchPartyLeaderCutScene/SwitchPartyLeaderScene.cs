...
namespace MyQuest
{
    public class SwitchPartyLeaderScene : Scene
    {
        Portal portal;

        public SwitchPartyLeaderScene(Screen screen)
            : base(screen)
        {
        }
        ...
        {
            Party.Singleton.SetPartyLeaderCara();

            Party.Singleton.RemoveFightingCharacter(Nathan.Instance);

            Party.Singleton.Leader.FaceDirection(Direction.South);
            portal = new Portal { DestinationMap = "blind_mans_town_inn_f1", DestinationPosition = new Point(4, 0), Position = Point.Zero };
        }
